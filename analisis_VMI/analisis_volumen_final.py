import time
import ROOT
#timeStamp (ms);flukeHighFlow (0.1 l/min);FlukePressureHF (0.01 mbar);FlukeVolumeHF (0.1 ml);FlukeVti (ml);FlukeVte (ml);FlukePeep (0.1 mbar);HF (l/min);PressureHF (cmH2O);VolumeHF (ml);Vti (ml);Vte (ml);Peep (cmH2O)
...
archivo_root = ROOT.TFile('prueba_volumen_final.root', 'RECREATE' )
mylist = list()
...
file  = open("Calibracionprueba_volumen_08-06-22.txt", "r")
...
while True:
    ...
    if a is None or len(a) == 0 or a is EOFError:
       ...
       volumen.Fill(tiempo-3993996,HF01,PF001,VolF01,VtiF,VteF,PeepmB,HF,PM,VolHF,Vti,Vte,Peep)
       i += 1
volumen.Write()

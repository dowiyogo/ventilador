...
int main() {
    ...
    const char* SEP = ",";
    const char* PARAMS = "flujo_neto:volumen_paciente:volumen_tidal:presion_paciente:" \
    ...
    TFile* file_data = new TFile("data_hosp.root","RECREATE");
    TNtuple* test_data = new TNtuple("test_data", "test data", PARAMS);

    // open file.
    FILE* fp;
    fp = fopen("ventilador_hosp.txt", "r");
    if (fp == NULL) {
